    ...
    ajaxPromise('module/shop/controller/controller_shop.php?op=details&id=' + id,
        ...
        .then(function (data) {
            ...
            $('<div></div>').attr('class', "details").html(
                '<table class="conf">' +
                '<tr>' +
                '<td>' + data[0].marca + ' ' + data[0].modelo + '</td>' +
                '<td>Category: ' + data[0].estado + '</td>' +
                '</tr><tr>' +
                '<td>Type: ' + data[0].tipo + '</td>' +
                '<td>City: ' + data[0].city + '</td>' +
                '</tr><tr>' +
                '<td>Fecha Matriculacion: ' + data[0].fecha_matr + '</td>' +
                '<td>Color: ' + data[0].color + ' </td>' +
                '</tr><tr>' +
                '<td>Cv: ' + data[0].cv + '</td>' +
                '<td>Precio: ' + data[0].precio + '</td>' +
                '</tr>' +
                '<div class="like" id=' + data[0].id + '><i id=' + data[0].id + ' class="fa-regular fa-heart fa-xl right regular dtlike"></i></div>'
            ).appendTo("#list");
            loadMap("map_details");
            addmarker(data[0]);
...
function sumcount(id) {
    ajaxPromise('module/shop/controller/controller_shop.php?op=sumcount&id=' + id,
        'GET', 'JSON');
}
...
function all_lists_products() {
    ...
    if (marca != null) {
        ...
        ajaxForSearch('module/shop/controller/controller_shop.php?op=filters&puertas=' + puertas + '&color=' + color + '&marca=a');
...
function loadorder() {
    ...
    $('.filterlist').on("click", "#order", function () {
        ...
        $.each($("input[name='order']:checked"), function () {
            ...
            ajaxForSearch('module/shop/controller/controller_shop.php?op=prueba&order=' + order + '&type=' + type + '');
            bootpag();
    ...
    ajaxPromise('module/shop/controller/controller_shop.php?op=count', 'POST', 'JSON')
        .then(function (data) {
            ...
            $('#page-selection').bootpag({
                ...
                ajaxForSearch('module/shop/controller/controller_shop.php?op=allcars&num=' + (localStorage.getItem('pag') * 5 - 5) + '');
            });
